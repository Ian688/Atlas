...
- **3D 代码城市**（`/city3d`）是同一份分析的第二种投影：目录为地块、文件为柱、函数为层、调用候选为地面管道。柱高不是「按数量等比」，而是**分段单调的尺度合同**：前 8 层线性、之后对数压缩并封顶、N=0 是矮柱、半径只做弱单调的第二通道。因为等比高度在真实项目上会撒谎——rxjs@7.8.1 上有函数的文件里 **92.1% 被画到最高柱的 1% 以下**（等于看不见），而一个 1056 函数的 bundle 会把整张图压平。同一尺度函数由 2D 与 3D 共用；页面上的「尺度」行给出标尺、中位柱/最高柱、不足最高柱 1% 的列数（并同时给出线性尺度下会是多少），可读性由 `scripts/bench_view_readability.py` 量测，不靠评价。未解析调用按事实画成琥珀矮桩而不丢弃，未分析的文件单独标识，覆盖与截断始终显示。渲染器是手写 WebGL2，无第三方依赖。
...
- **AI Coding 第一条完整链**（`atlas patch …`）：提案是统一 diff，先对**固定快照字节**在内存里应用，位置不符就带着不一致的那一行拒绝（不做模糊匹配——那会把改动挪到另一个长得像的地方）。形式由表头声明、由 hunks 复核：`--- /dev/null` 是**新建**、`+++ /dev/null` 是**删除**，两侧表头不一致即拒绝；删除不等于"编辑成空文件"（两者 revert 语义不同，记录里分开），新建必须指向还不存在的路径、删除必须指向存在的路径；git 的 `rename from/to` **具名拒绝**（把重命名写成删除+新建会丢掉两个路径之间的身份联系）。apply 与 revert 在「校验字节 + 写入」全程持有检出目录的 `.atlas-apply.lock`（第二个进程拿不到锁就被具名拒绝，错误路径也释放锁；崩溃残留的锁用 `atlas patch unlock --target <checkout>` 显式清理并报出原持有者；撤销来源是内容寻址的钉住 blob，没有额外磁盘备份，也没有合并）。按形式各查各的：新建要求那里什么都没有，删除要求磁盘上仍是提案所依据的字节，撤销新建会删掉文件、撤销删除会按钉住字节恢复。验证在**隔离副本**里重新索引：派生出新的 analysis，产出**图差异**（节点按 path+name 重新配对，因为 id 绑定字节区间，按 id 比较会把编辑读成删除+新增），并可在副本里跑一条**声明的 argv 测试命令**（不是 shell 字符串）。apply 会先校验目标当前字节仍等于提案所依据的固定快照，否则拒绝——覆盖审查之后发生的改动是损失而不是合并；revert 同样校验 apply 之后的字节。Intent（diff）、Static（重新派生的分析与图差异）、Observed（测试退出码）三类证据分开存放，没跑测试就写「没有跑任何测试，这不是通过」。
