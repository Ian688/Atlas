...
**阶段表述审计（2026-09-15）**：统一设计第 0 节明确愿景/目标架构/当前交付及四类要求。阶段范围不构成能力上限；当前路径中的结构性缺陷在本单修复。审计见 [整理报告](../evidence/documentation/2026-09-15-scope-architecture-audit/REPORT.md)。

**最新产品决定（2026-09-15，统一设计 v2）**：按 [产品设计](PRODUCT_DESIGN.md) 将地图、影响分析、场景验证、Agent 修改与审阅串成任务；统一设计已合并语义底座、局部失败、MCP 接入、任务/验收卡和三种 Review 视图。A 阶段继续当前 M1–M6，完成后优先 B1–B4 的任务与修改验收。C/D 的轨迹、调试等是后续设计，不能计入当前能力。
