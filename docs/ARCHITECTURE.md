...
当前 2D 概览画的是**共享形式层级的一个层级**，不再是"按加载顺序取前若干文件"。层级定义在 `web/hierarchy.js`，与 `/city3d` 读同一份：`项目 → 目录 → 文件`，同一份事实只被计数一次（目录与项目是子节点的和），`atlasLevelInvariants` 重新相加并在不守恒时列出字段。切换层级只改变"一个块代表什么"：项目层级一根聚合块、目录层级每目录一根、文件层级每个已枚举文件一根；聚合块被明确标注为聚合，且**没有单一源码可读**，点击它不会打开其中某一个文件。三个边界读数被分开报告，互不掩盖：**本页只加载了一页对象**（`本页已加载 100/8938 对象`）、**层级省略**（"这一层不展开文件"是层级定义）、**预算截断**（文件层只画前 60 个文件是渲染预算）。对象/关系仍显式分页；选中函数发起真实 source/reach 查询，保留高亮并静默其他对象，过期选择响应以递增请求版本丢弃。画布上的点选走 `atlasSpatialIndex`（均匀网格）而不是逐块扫描，命中、代价（`scanned`）与**具名未命中**（`outside_the_index` / `no_block_at_this_point`）都可被检查。

**调用视图是布局出来的，不是排出来的。** 选中函数后的焦点图先由 Atlas 建成**有界模型**（按到目标的距离分层、未解析目标保留为边界桩、超出预算的成员折叠并带成员清单），再由**钉住版本的布局引擎**（`web/vendor/elk.bundled.js`，elkjs 0.12.0，EPL-2.0，未修改，坐标以外不参与任何语义）给出坐标。每条边从**自己的端口**出发，不是从盒子边缘中点；因预算折叠的成员画成**摘要边**（`经 N 个函数` / `折叠 N 个成员`，虚线，`declared: folded_chain|folded_tail`）——**绝不允许读成直接调用**。状态行给出引擎、`交叉`、`标签碰撞`、`折叠`与预算，所以 2D 的可读性也成了数字。

分工是这条接缝的全部意义：**存在什么、什么是未知、预算裁掉了什么、选区带什么身份**属于 Atlas；**盒子放哪里**不属于。引擎不可用、抛错、或**返回没有坐标的结果**时，本地分层回退会接管，并且结果里标明 `engine: fallback_local` 与失败原因——一个看起来像布局的回退比一个承认的回退更糟。`elk_returned_no_coordinates` 是被实测逼出来的：同一个图同一个 API 在一个宿主里返回坐标、在另一个宿主里一个都不返回；若把缺失坐标当成 0，画出来的就是"所有盒子叠在原点"——一张看起来像有意为之、实际什么都没说的图。

布局是**异步**的（elkjs 0.12 没有同步 API），因此"迟到的布局不得覆盖新选区"是真问题：每次渲染递增 generation，过期的结果被丢弃并具名（`engine: 'discarded'`）而不是静默采用。当前布局在**主线程**上对有界焦点图执行（实测 21 块约 230 ms，含首次引擎初始化；`scripts/bench_view_layout.mjs` 在纯 node 里测得 30–115 ms），规格要求的 layout worker、deadline 与取消**尚未实现**，node 上限 `LAYOUT_MAX_NODES = 120` 是现在的边界。
...
**柱高是一条合同，不是等比缩放。** `web/hierarchy.js` 里的 `atlasScaleHeight / atlasScaleTier / atlasScaleRadius / atlasScaleReport` 是两个投影共用的尺度定义：N=0 是矮柱（显示文件角色）、前 8 层线性（每层可辨）、之后对数压缩并封顶，半径只做弱单调的第二通道（按构造饱和，所以它不可能变成读者真正比较的东西）。压缩柱会画出一条明确的**压缩标记环**，因为"没被标出的压缩高度"就是一个更好看的错误数字。页面上的**尺度行**给出标尺（`0=0.45层 · 1=1.00层 · 8=8.00层 · 20=11.97层 · 50=15.93层 · 1056（封顶）=29.13层`）、中位柱/最高柱、不足最高柱 1% 的列数，并且**同时给出线性尺度下会是多少**——不是让人记住旧行为，而是把它摆在旁边。

这不是审美选择：实测 rxjs@7.8.1，等比高度把 923 个"有函数的文件"里的 **850 个（92.1%）画到最高柱的 1% 以下**，等于不存在，同时让一个 1056 函数的 bundle 压平整张图。改进后的同一判据是 **0/923**，中位柱/最高柱从 0.003 升到 0.103。判据本身由 `scripts/bench_view_readability.py` 检查，并且**要求它自己能被证伪**：被替换掉的等比尺度必须在同一条判据上失败，否则这项检查就是装饰。该尺度公式只有一份实现，脚本通过 `scripts/scale_report.mjs` 读取它，不在 Python 里重抄一遍。
...
**已知缺口（不得读成已实现）**：3D 目前是透视 3/4 视角而不是规格建议的正交/平面 2.5D；柱体不透明（`alpha:false` + `vec4(...,1.0)`），没有规格要求的玻璃外壳；跨目录调用没有**边界端口**，只有块心直连；相机与折叠不跨视图保存；页面常驻 `requestAnimationFrame` 而非"静态阅读按需重绘"；没有搜索定位到层、没有类层组标题、没有 `visibility_reason` 枚举、没有文件树/全局小图/时间轴。2D 仍没有真正的分层布局、端口与摘要边语义。

**两个投影只有一个层级定义。** 层级本身是几何无关的：`web/hierarchy.js` 只知道身份、成员关系、被计数的事实与层级分组，不知道柱子、地块、像素或相机；每个投影各自在其上添加几何和自己的绘制预算。这条边界是被检查的，不是被承诺的：`web/city3d.js` 与 `web/app.js` 都不得再定义 `ATLAS_LEVELS` / `ATLAS_FACT_KEYS`，两个页面都必须先加载 `/hierarchy.js`（`web/tests/app.behavior.test.mjs` 中的静态断言），且 `web/hierarchy.js` 计入门禁指纹——否则改动共享定义可能不改变指纹，那等于门禁看不见它。
...
**apply/revert 持锁**：整个「校验字节 + 写入」过程持有检出目录里的 `.atlas-apply.lock`（`create_new`，由操作系统保证第二次创建失败；文件里写着持有者与取锁时刻）。没有它，两个进程可以都通过「检出仍是审查过的字节」然后都写，第二个赢——而它的检查描述的已经是不存在的检出了。锁在 guard drop 时释放，**包括每一条错误路径**（有用例检查被拒绝的 apply 不留锁）。撤销来源就是内容寻址的钉住 blob（读取时重新哈希校验），所以没有额外的磁盘备份副本；**合并与逐文件备份仍然没有**。崩溃残留的锁会被下一次 apply 报出来（宁可挡一次也不静默消失），清理用 `atlas patch unlock --target <checkout>`：显式、报出原持有者与取锁时刻、没有锁时以 `no_apply_lock` 报错而不是静默成功；旧格式锁文件没有时间戳时如实报 `since_ms=null`（不猜）。
