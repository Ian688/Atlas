...
**apply/revert 持锁**：整个「校验字节 + 写入」过程持有检出目录里的 `.atlas-apply.lock`（`create_new`，由操作系统保证第二次创建失败；文件里写着持有者身份）。没有它，两个进程可以都通过「检出仍是审查过的字节」然后都写，第二个赢——而它的检查描述的已经是不存在的检出了。锁在 guard drop 时释放，**包括每一条错误路径**（有用例检查被拒绝的 apply 不留锁）。撤销来源就是内容寻址的钉住 blob（读取时重新哈希校验），所以没有额外的磁盘备份副本；**合并与逐文件备份仍然没有**。
