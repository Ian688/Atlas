...
实际接口：CLI `index/report/nodes/edges/reach/source/context/flows/flow/serve`；HTTP GET `report/nodes/edges/reach/source/flow/flows`，POST `context`。均由引擎查询同一分析版本。HTTP 不开放索引任意路径、写代码、执行程序或访问生产数据库。
