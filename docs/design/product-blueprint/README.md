...
前端图布局、源码与 diff、测试报告及调试优先采用合适的成熟组件。图中的关联对象是产品语义；按统一设计第 0 节保持模型与接口可扩展，以实际实现和样本证明接入点。
...
愿景与目标架构覆盖全部阶段；每阶段的交付范围与架构责任分别检查，见 [统一设计第 0 节](../../PRODUCT_DESIGN.md)。A 已有在途实现，先检查与补缺。B 的完整金额边界任务比增加装饰和菜单更优先。D 的独立能力不需要等所有轨迹问题解决。
