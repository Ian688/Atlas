...
const BASE = '/var/folders/d0/h8zmbc0971n266zb4nx3j9980000gn/T/atlas-independent-hgw1dvk7';
...
try {
  ...
  await page.locator('.wb-tabs [data-mode="run"]').click();
  await page.waitForSelector('#exec-param-0');
  const movedInputs=[await page.locator('#exec-param-0').inputValue(),await page.locator('#exec-param-1').inputValue()];
  record('relocated function restores actual draft inputs', movedInputs[0]==='17'&&movedInputs[1]==='23',{movedInputs,status:moved.status});
