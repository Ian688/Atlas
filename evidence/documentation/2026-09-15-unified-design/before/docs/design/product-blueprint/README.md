...
> 先按仓库 AGENTS.md 读取当前任务书与交接，再读本图谱和 docs/PRODUCT_DESIGN.md。图 01 明确目的，图 02–03 明确页面与点击行为，图 04 是贯穿用户任务，图 05–06 明确真实数据来源，图 07 是阶段顺序。当前完成 A/M1–M6；已经实现的功能直接验证和补缺，不重复建设。A 完成后按产品设计推进 B，不在同一轮扩张到全部轨迹和调试。每个功能交付真实页面行为与数据链，保留项目/版本身份；缺数据时给准确状态。报告实际完成的用户流程、未完成项与证据，不用图上的功能名称当作完成证明。
