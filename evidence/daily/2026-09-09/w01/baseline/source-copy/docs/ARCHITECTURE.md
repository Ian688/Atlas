...
默认预算为 20,000 个清单项、每文件 2 MiB、累计捕获 64 MiB。超大文件留下清单项，累计/条目预算超限不发布不完整 Snapshot。已写但未引用的 blob 可留下，尚未实现 GC。捕获的非 UTF-8 源文件保留字节并报告跳过原因。
...
**未实现**：完整语义诊断/类型检查、所有 CommonJS/re-export 关系清点、动态 import、闭包环境/TDZ/函数执行时间、this/继承/堆别名、反射、框架依赖注入、跨语言联系、可靠调用目标集合、函数参数与返回的数据流。候选关系不能证明目标会被执行或只可能执行它。纯源码解析也不会告诉用户某次复制“实际复制了哪些字节”。
...
实际接口：CLI `index/report/nodes/edges/reach/source/context/serve`；HTTP GET `report/nodes/edges/reach/source`，POST `context`。均由引擎查询同一分析版本。HTTP 不开放索引任意路径、写代码、执行程序或访问生产数据库。
...
以下为设计要求，尚未实现，不添加空方法冒充可调用能力。

| 子系统 | 完整能力与算法方向 | 与当前基础的接缝 |
|---|---|---|
| 语言中立流 IR | CFG block、typed edges、作用域/变量、求值顺序、异常 completion；语法到 IR 的可复验来源 | LanguageFacts 增量版本，不把当前 calls 当作 CFG |
| 本地数据流 | 格上抽象值、环境/堆、转移函数、工作表固定点、widen/narrow、points-to/alias、字段敏感度与预算未知 | Rust 派生独立 facts_kind、依赖、精度/覆盖 profile |
