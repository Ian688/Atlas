...
export function parse(request) {
  ...
  const result = { schema: 'atlas.language-facts.v1', snapshot_id: request.snapshot_id,
    producer: `typescript/${ts.version};worker/0.2.0`, parsed_files: [], symbols: [], calls: [], imports: [], diagnostics: [], dynamic_files: [] };
  ...
  for (const sf of program.getSourceFiles()) {
    ...
    function walk(node, owner) {
      if (ts.isFunctionLike(node) && node.body) {
        ...
        const record = {id,path:p,name:name.slice(0,256),kind:'function',start,end,container:owner,mutated:false,node};
        result.symbols.push(record); functions.set(node, record); declarationIds.set(node, id);
        if (!recordsByFile.has(sf)) recordsByFile.set(sf, []);
        recordsByFile.get(sf).push(record);
  ...
  for (const sf of program.getSourceFiles()) {
    ...
    function walk(node, owner) {
      ...
      if (ts.isWithStatement(node) || (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && ['eval','Function'].includes(node.expression.text))) dynamic = true;
  ...
  // Versioned Flow IR for every extracted function body (W01 profile).
  const filesContext = new Map();
  for (const [sf, offsets] of sourceMaps) filesContext.set(sf, { sf, offsets, path: relative(sf.fileName) });
  result.flow = buildFlow({ snapshotId: request.snapshot_id, checker, functions, recordsByFile, files: filesContext });
  for (const record of result.symbols) delete record.node;
