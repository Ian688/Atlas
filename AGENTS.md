...
- Current priority: the 2D function workbench, from finding a function through understanding it and running it. The work order defines acceptance and the next task. Milestones define delivery, not architectural limits. Apply the target-architecture table in docs/PRODUCT_DESIGN.md: indexed/lazy queries, versioned identity, language adapters, incremental correctness, cross-file relations, transport-independent services and runtime evidence. Broader repository qualification and Modus integration progress along that architecture. Fix structural defects exposed by the current user journey within the active task.
